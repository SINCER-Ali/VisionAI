...
pub fn confusion_matrix(predictions: &[usize], targets: &[usize], num_classes: usize) -> Vec<Vec<usize>> {
...
pub fn precision(matrix: &[Vec<usize>], class: usize) -> f64 {
    ...
    let fp: usize = (0..matrix.len()).filter(|&i| i != class).map(|i| matrix[i][class]).sum();
...
pub fn recall(matrix: &[Vec<usize>], class: usize) -> f64 {
    let tp = matrix[class][class];
    let fn_: usize = (0..matrix.len()).filter(|&j| j != class).map(|j| matrix[class][j]).sum();
