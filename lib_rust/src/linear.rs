...
impl LinearModel {
    ...
    pub fn new(input_dim: usize) -> Self {
        // Constructeur : cree un modele pour des entrees de taille input_dim
        ...
        for _ in 0..(input_dim + 1) {
            // Pour chaque poids (y compris le biais)
    ...
    pub fn predict_class(&self, x: &[f64]) -> f64 {
        // Donne la classe prédite (+1 ou -1)
        if self.predict_value(x) >= 0.0 {
            // Si la valeur prédite est positive ou nulle
            1.0 // Classe +1
        } else {
            // Sinon
            -1.0 // Classe -1
    ...
    pub fn train(
        // Entraine le modèle sur un ensemble d'exemples
        ...
        for _ in 0..epochs {
            ...
            for k in 0..n_samples {
                ...
                for i in 0..input_dim {
                    // Pour chaque entrée de l'exemple
