...
use core_lib::models::{Model, TrainConfig, Vector};
...
async fn predict(
    ...
    let model_name = req.model_name.unwrap_or_else(|| "default".to_string());
    ...
    let input = image_to_vector(&bytes, get_input_size(model));
    let (class, confidence) = run_prediction(model, &input);
...
fn run_prediction(model: &StoredModel, input: &Vector) -> (String, f64) {
    match model {
...
fn image_to_vector(bytes: &[u8], input_size: usize) -> Vector {
    let mut data = vec![0.0; input_size];
    if bytes.is_empty() {
        return data;
    }

    for (i, b) in bytes.iter().enumerate() {
        let idx = i % input_size;
        data[idx] += (*b as f64) / 255.0;
    }
